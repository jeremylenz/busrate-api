  ...
  def self.aggregate_trip_view(departures)
    ...
    sorted_departures.each_instance do |current_departure|
      ...
      if current_departure.trip_identifier == current_batch_trip_identifier
        ...
        directions = current_batch.map { |d| d.direction_ref }  # [0, 0, 1, 1, 1]
        # All departures in the batch must be the same direction - pick whichever direction has more departures
        ...
        if sample_departure.present?
          ...
          # All departures in the batch must have the same vehicle_ref
