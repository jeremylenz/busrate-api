class HistoricalDeparture < ApplicationRecord
  ...
  def self.calculate_headways(unsorted_historical_departures)
    ...
    HistoricalDeparture.transaction do
      ...
      cursor = unsorted_historical_departures.lock.order("stop_ref, line_ref, departure_time DESC").each_instance(symbolize_keys: true) do |hd|
        hd_hash = {
        :id => hd.id,
        :headway => hd.headway,
        ...
        puts hd_hash
        puts lka
        puts hd.departure_time.class
        puts lka[:departure_time].class
  ...
  #   deps = unsorted_historical_departures.order("stop_ref, line_ref, departure_time DESC") # make sure it's sorted
